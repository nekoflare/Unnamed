...
void memory::init_kernel_virtual_allocator() noexcept {
    ...
    std::uintptr_t range_start = core::align_down(get_memory_slide() + get_highest_physical_address(), PAGE_SIZE);
    std::uintptr_t range_end = core::align_down(get_kernel_base_address(), PAGE_SIZE);
    std::uintptr_t range_length = range_end - range_start;

    allocator.init(range_start, range_length);

    logger::debug("Free virtual memory: %lu (B)", range_length);
}

std::optional<std::uintptr_t> memory::allocate_virtual_memory(std::size_t length) {
...
void memory::deallocate_virtual_memory(std::uintptr_t address, std::size_t length) {
