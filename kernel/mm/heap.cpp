...
extern "C" {
    ...
    void* mmap(void* addr, std::size_t length, int prot, int flags, int fd, intptr_t offset) {
        (void) addr;
        (void) prot;
        (void) offset;
        (void) fd;

        if (!core::is_flag_set(flags, MAP_PRIVATE) || !core::is_flag_set(flags, MAP_ANONYMOUS)) {
    ...
    long unsigned int sbrk(ptrdiff_t increment) {
        ...
        std::size_t aligned_increment = core::align_up(static_cast<std::size_t>(increment), PAGE_SIZE);
...
void memory::init_heap() {
    ...
    if (address.has_value()) {
        heap_base = reinterpret_cast<std::uintptr_t>(address.value());
    } else {
        core::set_error(core::ErrorCode::OutOfMemory);
